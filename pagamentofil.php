...
$tituloPagina = "Pagamento Filiado";

$afiliado_matricula = $mes = $ano = $unimed = $uniodonto = $rcs = $das = $message = '';
...
?>
  ...
  <div class="panel-heading">
    <h2 class="panel-title"><big>Pagamento de Convênios do Filiado</big></h2>
  </div>
  ...
  <?php
    ...
    if(empty($afiliado_matricula)||empty($mes)||empty($ano)){
      ...
      if(pagamentoFiliado($afiliado_matricula, $ano, $mes, $das, $rcs, $unimed, $uniodonto)){
      $message = mesSucesso("Pagamento realizado com sucesso!");
      //Limpa o formulário
      $afiliado_matricula = $mes = $ano = $unimed = $uniodonto = $rcs = $das = '';
      }
    }
  }
  ...
  ?>
    ...
    <form class="form-horizontal" data-toggle="validator" role="form" method="post" action="pagamentofil.php">
